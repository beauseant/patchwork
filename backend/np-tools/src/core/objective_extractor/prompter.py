...
class Prompter:
    def __init__(
        ...
        config_path: pathlib.Path = pathlib.Path("/np-tools/src/core/objective_extractor/config/config.yaml"),
        ...
        if max_tokens is not None:
            ...
            if model_type in self.GPT_MODELS or model_type in self.VLLM_MODELS:
                ...
                self._logger.info(
                    f"Setting max_completion_tokens to: {max_tokens}")
    ...
    def _call_ollama_api(template, question, model_type, params, context):
        ...
        if Prompter.ollama_client is None:
            raise ValueError(
                "OLLAMA client is not initialized. Check the model type configuration.")
    ...
    def _call_llama_cpp_api(template, question, params, llama_cpp_host="http://kumo01:11435/v1/chat/completions"):
        ...
        if response.status_code == 200:
            ...
            raise RuntimeError(
                f"llama_cpp API error: {response_data.get('error', 'Unknown error')}")
