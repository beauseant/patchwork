document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadCorpus() {
        ...
        if (corpusList) {
            populateSelect(selectCorpus, corpusList, 'Selecciona un corpus');
    ...
    async function loadCPVs() {
        ...
        resetSelect(selectCPV, 'Cargando CPVs...');
        resetSelect(selectGranularity, 'Esperando CPV...');
        clearTreemap();
        ...
        if (cpvData) {
            ...
            populateSelect(selectCPV, cpvKeys, 'Selecciona un CPV');
    ...
    function loadGranularity() {
        ...
        // Resetear
        resetSelect(selectGranularity, 'Cargando granularidad...');
        clearTreemap();
        ...
        try {
            ...
            if (granularities.length > 0) {
                populateSelect(selectGranularity, granularities, 'Selecciona granularidad');
                selectGranularity.disabled = false;
            } else {
                resetSelect(selectGranularity, 'No hay granularidad');
            }
        } catch (error) {
            console.error("Error al parsear granularidad:", error, currentCorpusData);
            resetSelect(selectGranularity, 'Error al cargar');
        }
    }
    ...
    function handleNodeClick(event, data) {
        ...
        modalLabel.textContent = `Detalles: ${data.tpc_labels} (ID: ${data.id})`;
        ...
        const stats = {
            ...
            'Documentos Activos': data.ndocs_active
        };
