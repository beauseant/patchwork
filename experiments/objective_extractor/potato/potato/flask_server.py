...
def load_all_data(config):
    ...
    link_key = config['item_properties']["link_key"]
    ...
    for inst_id in instance_id_to_data:
        instance_id_to_data[inst_id]["displayed_text"] = get_displayed_text(
            ...
            instance_id_to_data[inst_id][config["item_properties"]["context_key"]],
            instance_id_to_data[inst_id][config["item_properties"]["link_key"]],
        )
...
def get_displayed_text(
    ...
    *,
    ...
    right_text=None,
    left_title="Opción A",
    right_title="Opción B"
):
    """
    Returns HTML:
      [Link] -> [Scrollable Context] -> [Two colored boxes: Left | Right]

    Auto-mapping rules (only used if left_text/right_text are not passed):
      - If `text` is a list of length 2: text[0] -> left, text[1] -> right
      - If `text` is a dict with exactly 2 keys: first -> left, second -> right
      - Else: everything goes in the RIGHT box and the left shows a placeholder
    """

    # --- keep your unfolding / formatting logic for `text` (for non-2-item cases) ---

    orig_text = text  # keep raw for 2-item mapping
    text = text.split("||")
    ...
    if "list_as_text" in config and config["list_as_text"]:
        ...
        if isinstance(text, list):
            # Only format as joined string if not using it for 2-box mapping
            if len(text) != 2:
            ...
            if "randomization" in config["list_as_text"]:
                if config["list_as_text"].get("randomization") == "value":
                    ...
                    text = {key: value for key, value in zip(list(text.keys()), values)}
                    ...
                    print(
                        "WARNING: %s currently not supported for list_as_text, please check your .yaml file"
                        % config["list_as_text"].get("randomization")
                    )

            # If more than 2 keys, render vertically (legacy behavior)
            if len(text) != 2:
                block = []
                for key in text:
                    block.append(
                        f'<div id="instance-text" name="instance_text"><legend>{key}</legend> {text[key]} <br/></div>'
                    )
                text = "".join(block)
        else:
            text = text

    # --- derive left/right content if not explicitly passed ---
    # Prefer explicit params; otherwise infer from `orig_text` when it looks like 2 options.
    if left_text is None and right_text is None:
        if isinstance(text, list) and len(text) == 2:
            left_text, right_text = text[0], text[1]
        elif isinstance(text, dict) and len(text) == 2:
            # preserve insertion order (Python 3.7+)
            keys = list(text.keys())
            left_title, right_title = str(keys[0]), str(keys[1])
            left_text, right_text = text[keys[0]], text[keys[1]]
        else:
            # fallback: everything goes to right box; left shows placeholder
            right_text = text if isinstance(text, str) else (str(text) if text is not None else "")
            left_text = "<em>No left option provided</em>"

    # Make sure context/link exist
    context_html = context if context else "<em>No context provided</em>"
    link_html = f'<div class="meta"><a href="{link}" target="_blank" rel="noopener noreferrer">{link}</a></div>' if link else ""

    # Styles and layout
    style = """
    <style>
      :root {
        --gap: 16px; --radius: 14px; --border: #e5e7eb; --ink: #111827; --muted: #6b7280;
        --brand: #2563eb; --bg: #f9fafb;
        --left-bg: #ecfdf5;   /* green-ish */
        --left-border: #34d399;
        --right-bg: #eff6ff;  /* blue-ish */
        --right-border: #60a5fa;
      }
      .meta { margin: 10px 0 14px 0; font-size: 14px; word-break: break-word; }
      .meta a { color: var(--brand); text-decoration: none; }
      .meta a:hover { text-decoration: underline; }

      .context-card { background: #fff; border: 1px solid var(--border); border-radius: var(--radius); box-shadow: 0 1px 2px rgba(0,0,0,.05); }
      .context-card header { padding: 14px 16px; border-bottom: 1px solid var(--border); text-transform: uppercase; font-size: 13px; color: #374151; letter-spacing: .2px; }
      .context-scroll { max-height: 220px; overflow: auto; padding: 14px 16px; line-height: 1.55; }

      .options { display: flex; gap: var(--gap); margin-top: 16px; flex-wrap: wrap; }
      .option { flex: 1 1 360px; border-radius: var(--radius); border: 2px solid transparent; box-shadow: 0 1px 2px rgba(0,0,0,.05); }
      .option header { padding: 12px 14px; font-weight: 600; text-transform: uppercase; font-size: 12px; letter-spacing: .3px; opacity: .9; }
      .option .content { padding: 14px 16px; line-height: 1.6; }

      .option-left  { background: var(--left-bg);  border-color: var(--left-border); }
      .option-right { background: var(--right-bg); border-color: var(--right-border); }
    </style>
    """
    ...
    {style}
    {link_html}
    <section class="context-card" aria-label="Context">
    <header>Título (objeto del contrato) original de PLACE</header>
    <div class="context-scroll">{context_html}</div>
    </section>

    <div class="options">
      <section class="option option-left" aria-label="Left option">
        <header>{left_title}</header>
        <div class="content">{left_text if left_text else "<em>Empty</em>"}</div>
      </section>

      <section class="option option-right" aria-label="Right option">
        <header>{right_title}</header>
        <div class="content">{right_text if right_text else "<em>Empty</em>"}</div>
      </section>
    </div>
    """
    return layout
...
def run_server(args):
    ...
    print("running at:\nlocalhost:" + str(port))
    app.run(debug=args.very_verbose, host="0.0.0.0", port=port, ssl_context=ssl_context, use_reloader=False)
