...
echo "python path: $REPO_ROOT/backend/np-tools:${PYTHONPATH}"
  ...
  for file in "$source_directory"/*; do
    if [ -f "$file" ]; then
      ...
      if [ -e "$destination_file" ]; then
        echo "The file '$destination_file' already exists in the destination directory, skipping."
      else
        echo "The file '$destination_file' did not exist, processing it."
